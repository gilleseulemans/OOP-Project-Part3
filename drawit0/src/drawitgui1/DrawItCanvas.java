		...
		addMouseListener(new MouseAdapter() {
				...
				if (e.getClickCount() == 2) {
					if (selectedShapes.size() == 1) {
						...
						if (selectedShape instanceof RoundedPolygonShape) {
							p = selectedShape.toShapeCoordinates(p);
			...
			public void mousePressed(MouseEvent e) {
				...
				if (selectedShapes.size() == 1) {
					...
					IntPoint shapePoint = selectedShape.toShapeCoordinates(p);
					...
					if (controlPointIndex != -1) {
						...
						MouseAdapter ma = new MouseAdapter() {
							...
							public void mouseDragged(MouseEvent e1) {
								...
								catchErrors(() -> controlPoint.move(q.minus(p)));
							}
						};
						addMouseListener(ma);
					...
					if (selectedShape instanceof ShapeGroupShape) {
						...
						if (group instanceof LeafShapeGroup) {
							...
							if (polygon.contains(group.toInnerCoordinates(p))) {
								selectedShapes.clear();
							...
							ShapeGroup subgroup = ((NonleafShapeGroup)group).getSubgroupAt(group.toInnerCoordinates(p));
							if (subgroup != null) {
								selectedShapes.clear();
	...
	protected void paintComponent(Graphics g) {
		...
		for (Shape selectedShape : selectedShapes) {
			...
			for (ControlPoint controlPoint : controlPoints) {
				IntPoint location = selectedShape.toGlobalCoordinates(controlPoint.getLocation());
